...
![](../../.gitbook/assets/c2.png) 形参和实参： 1.形参在函数未调用时是不占用内存中的存储单元的。**只有在发生调用函数时，函数的形参才被分配内存单元，函数结束后形参所占的内存单元会被释放** 2.在函数调用时，会将实参的值赋给形参。 3.实参与形参类型应相同。 4.实参与参数之间是单项传递，只能由实参传递给形参，而不能由形参传回来给实参。

## return的意义

1. 返回数据类型
2. 结束所在函数 我记得有个问题是如何结束一个多层循环，那么有时候我们就可以通过return结束函数来结束多层循环，这里有很多方法，我们到时就按照自己的需求来用。比如：还可以用标签，goto这些，或者设置一个flag在循环的控制条件里。
