...
比如100000000，这里是1亿，这样就很浪费资源了，用10^8其实表示的是同一个。这个压缩其实是有一个公式： ![](../.gitbook/assets/yasuo.png)
...
格式：tar \[主选项+辅选项\] 文件或目录 主选项：它们不可能同时出现，告诉tar要做什么事，是必须要有的 -c：创建新的压缩文件，相当与打包 -x：从压缩文件中释放文件，相当于拆包 -t：列出压缩文件中的内容 辅选项：可选 -C：切换到指定目录 -z：是否同时具有gzip的属性？亦即是否需要用gzip压缩或解压？一般格式为tar.gz或xx.tgz -j：是否同时具有bzip2的属性？亦即是否需要用bzip2压缩或解压？一般格式为tar.bz2 -v：压缩过程中显示文件 -f：指定压缩文件，注意，在f之后要立即接档名（自己取的），不要再加其他参数 -p：使用原文件的原来属性 -exclude FILE：在压缩过程中，不要将FILE打包
