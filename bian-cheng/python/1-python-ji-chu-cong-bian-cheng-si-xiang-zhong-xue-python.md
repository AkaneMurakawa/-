...
**代码规范** 1. 比如变量的命名规则？函数的命名规则？文件的命名？ 看看别人大神是怎么写的，比如可以到github去查找python项目 2. 模块的分布，比如说你不可能所有代码都写在一个文件，那你就需要按照一种规则是分布 3. 符号和字母之间，后面要加空格，前面不用。
...
print\(\) input\(\) 注: 如果想转为int，要用函数去转换， int\(input\(''\)\) type\(\) len\(\) max\(\) min\(\)
...
2. 可变参数
   ...
   注: var\_args\_tuple是tuple

def functionname\(\[formal\_args,\] \*\*var\_args\_dict \): 注: var\_args\_dict是dict
...
两种导入的方式： 1. import 文件名\(.py\) 全部内容一起导入 2. from 文件名\(.py\) import 要导入的模块或者函数 导入指定的 注：这样的话就可以快一点
...
Q: 什么打开模式是在内容后添加，什么是删除全部内容重新添加？ A: 即原有内容会被删除: `w、wb、wb+、w+` 不会被删除的，而是在文件的结尾添加的：`a、ab、ab+、a+`\(add\)
