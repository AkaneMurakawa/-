...
那时候我学C的时候问题没有那么多，现在重新学习反而对这些很有兴趣。C语言数组中的a, &a, &a\[0\]是不是傻傻分不清楚？地址的加法又是怎么回事？ 其实也没有那么难，一维数组的地址加1其实加上**一个类型**的大小，并不是普通的加1。你要明白我们说的数组的地址是首元素的首地址。

**而数组名a是一个常量，首元素的地址。&a是数组的地址，相当于小区。&a\[0\]是首元素的首地址，相当于住户，单个元素的地址** **Q：&a和&a\[0\]的区别？** **A:** 1.小区地址与具体用户的地址 2.&a+1加上一个类型\(整个数组\)的大小，**一个小区的下一个，是下一个小区** 3.&a\[1\]+1加上一个类型\(一个元素\)的大小，**住户的下一个，是下一户。**
...
下图是我看C3程序员的教程截取的图，如有冒犯立删。从图中对二维数组的描述，我们就能更好的理解这个地址加1的意义了。 ![](../../.gitbook/assets/pro07-数组的地址png.png)
